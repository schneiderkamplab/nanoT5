...
class T5SequenceNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6, momentum=0.1):
        ...
        # Initialize running statistics as scalars
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            # Compute per-sample mean and variance over sequence and embedding dimensions
            mean = hidden_states.mean(dim=(1, 2), keepdim=True)  # Shape: (batch_size, 1, 1)
            var = hidden_states.var(dim=(1, 2), keepdim=True, unbiased=False)  # Shape: (batch_size, 1, 1)

            # Update running statistics per sample
            self.running_mean = (1 - self.momentum) * self.running_mean + self.momentum * mean.mean(dim=0).detach()
            self.running_var = (1 - self.momentum) * self.running_var + self.momentum * var.mean(dim=0).detach()

        else:  # Inference mode
            # Use running statistics during inference
            mean = self.running_mean
            var = self.running_var
            if hasattr(self, '__qualname__') and "final" in self.__qualname__:
                print(f"{self.__qualname__}: mean = {mean}, var = {var}")

        # Normalize hidden states using the calculated or running statistics
...
class T5DenseGatedActDense(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.layer_norm = T5SequenceNorm(config.d_ff, eps=config.layer_norm_epsilon)
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = self.layer_norm(hidden_states).type_as(hidden_states)
...
class T5LayerFF(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.layer_norm = T5SequenceNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.layer_norm = T5SequenceNorm(self.inner_dim, eps=config.layer_norm_epsilon)
    ...
    def forward(
        ...
        attn_output = self.layer_norm(attn_output).type_as(attn_output)
...
class T5Stack(nn.Module, ModuleUtilsMixin):
    def __init__(self, config, embed_tokens):
        ...
        self.final_layer_norm = T5SequenceNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
