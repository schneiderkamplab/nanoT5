...
class T5BatchNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-5, momentum=0.1):
        super().__init__()
        # BatchNorm1d expects (N, C, L) format
        self.batch_norm = nn.BatchNorm1d(hidden_size, eps=eps, momentum=momentum)
...
class T5DenseGatedActDense(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.layer_norm = T5BatchNorm(config.d_ff, eps=config.layer_norm_epsilon)
...
class T5LayerFF(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.layer_norm = T5BatchNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.layer_norm = T5BatchNorm(self.inner_dim, eps=config.layer_norm_epsilon)
...
class T5Stack(nn.Module, ModuleUtilsMixin):
    def __init__(self, config, embed_tokens):
        ...
        self.final_layer_norm = T5BatchNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
